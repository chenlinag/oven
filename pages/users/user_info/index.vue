			...
			<view class='personal-data'>
				<view class="wrapper" v-if="switchUserInfo.length>0">
					...
					<view class="wrapList" >
						<view class="item acea-row row-between-wrapper" :class="item.uid === userInfo.uid ? 'on' : ''"
						 :style="{ 'background-image': `url(${item.uid == userInfo.uid ? headBg+keyColor+'.png' : '' })` }"
						v-for="(item,index) in switchUserInfo"
						 :key="index">
							<view class="picTxt acea-row row-between-wrapper">
								<view class="pictrue" v-if='item.uid === userInfo.uid' @click="uploadpic">
								...
								<view class="pictrue" v-else>
									<image  :src='item.avatar' v-if="item.avatar"></image>
									<image src="/static/images/f.png" v-else></image>
									<view class="radius" v-else-if="open_update_info != 0"><span class="iconfont iconfonta icon-jilu-xuanzhong"></span></view>
								</view>
								<view class="text">
				...
				<view class="list" v-else>
					<view class='item acea-row row-between-wrapper' >
						<view>头像</view>
						<view class="pictrue" @click="uploadpic">
				...
				<view class='list' style="margin-top: 1rpx;">
					<view class='item acea-row row-between-wrapper'>
						...
						<view class='input acea-row row-between-wrapper' @click="changeInput">
							<input type='text'  name='nickname'  :value='userInfo.nickname' disabled class="id"></input>
