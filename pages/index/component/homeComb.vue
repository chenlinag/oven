<template>
	<view class="page_count">
		...
		<view :class="{scrolled:isScrolled, 'my-main': true}">	
				...
				<view class="header" :style="'background: '+ bgColor +';margin-top:'+ mbConfig +'rpx;'">
					...
					<view class="serch-box tui-skeleton">
						<view class="serch-wrapper flex">
							<view v-if="logoConfig" class="logo skeleton-rect"><image :src="logoConfig" mode="widthFix"></image></view>
							<navigator v-if="hotWords.length > 0" :url="'/pages/columnGoods/goods_search/index?searchVal='+searchVal" :class="logoConfig ? 'input' : 'uninput'"
							...
							<navigator v-else url="/pages/columnGoods/goods_search/index" :class="logoConfig ? 'input' : 'uninput'" 
			...
			<view style="visibility: hidden;" :style="{ height: navHeight + 'px' }"></view>
			<!-- #endif -->
			<view class="navTabBox tabNav" :class="{isFixed:isFixed}" :style="'top:'+isTop">		
...
<script>
	import { navigatoPage } from "../../../utils/index.js"
	...
	export default {
		...
		data() {
			return {
				...
				mbConfig: this.dataConfig.mbConfig ? this.dataConfig.mbConfig.val*2 : 0,
		...
		created() {
			...
			setTimeout((e) => {
				...
				query.select('.header').boundingClientRect(data => {
					that.isTop = data.height+'px'
					that.marTop = data.height
				}).exec();
				// #endif
			}, 300)
			// #ifdef MP || APP-PLUS
			that.isTop = (uni.getSystemInfoSync().statusBarHeight + 50) + 'px'
			//#endif
			// #ifdef H5
			// that.isTop = 0
			// #endif
			...
			setTimeout((e) => {
				...
				//#endif
				// #ifndef H5
				that.swiperTop = that.navHeight+that.homeTop/2+that.marTop+(uni.getSystemInfoSync().statusBarHeight + 43)+5 
				//#endif
			}, 500)
		},
		mounted(){
